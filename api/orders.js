...
const A='eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im15enJ0c2hiYWF6aHpybmphbGVvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODg2NDcwMTUsImV4cCI6MjEwNDIyMzAxNX0.ynwtLNLBVmnWB5NsJTcekvCypCreMKPla1yLQ23-ToU';
...
module.exports=async(req,res)=>{
  if(req.method==='POST'){
    try{
      ...
      const allowed=['Tortilla','Eggs','Hash Browns','Sausage','Bacon','Onions & Green Peppers','Sour Cream','Cheese','Salsa','Cholula'],ings={};
      for(const [k,v] of Object.entries(b.ingredients)){
        if(!allowed.includes(k))continue;
        if(k==='Tortilla'){if(['Yes','No'].includes(v))ings[k]=v}
        else if(['Light','Normal','Extra'].includes(v))ings[k]=v;
      }
  ...
  if(req.method==='PATCH'){
    ...
    try{
      ...
      if(deleted){await sb(`orders?id=eq.${encodeURIComponent(id)}`,{method:'DELETE'});return j(res,200,{ok:true})}
      if(!['active','complete'].includes(status))return j(res,400,{error:'Invalid status'});
      const data=await sb(`orders?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',body:JSON.stringify({status,completed_at:status==='complete'?new Date().toISOString():null})});
      return j(res,200,data?.[0]||{ok:true});
    }catch(e){return j(res,500,{error:e.message})}
  }
